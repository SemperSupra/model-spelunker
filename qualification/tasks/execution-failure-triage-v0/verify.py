...
def check(root: Path) -> bool:
    try:
        evidence = json.loads((root / "fixture/evidence.json").read_text(encoding="utf-8"))
        ...
        for cid, (expected_class, expected_probe) in EXPECTED.items():
            ...
            prefix = f"fixture/evidence.json#/cases/{cid}"
            resolved = []
            for ref in refs:
                if not isinstance(ref, str) or not ref.startswith(prefix):
                    return False
                pointer = ref[len("fixture/evidence.json#"):]
...
def good_output() -> dict:
    ...
    for cid, (klass, probe) in EXPECTED.items():
        refs = [
            f"fixture/evidence.json#/cases/{cid}{suffix}"
            for suffix in REQUIRED_EVIDENCE_SUFFIXES[cid]
        ]
        rows[cid] = {
            "class": klass,
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (root / "fixture/evidence.json").write_text(fixture_text, encoding="utf-8")
        ...
        bad["cases"]["c02"]["evidence_refs"] = [
            "fixture/evidence.json#/cases/c02/stderr"
        ]
        (root / "triage.json").write_text(json.dumps(bad), encoding="utf-8")
        assert not check(root)

    print("PASS execution-failure-triage verifier self-test")
    return 0
