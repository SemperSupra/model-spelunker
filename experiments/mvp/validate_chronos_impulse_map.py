...
def main():
 ...
 assert b["probe_id"]=="chronos-bolt-impulse-response-map-v1" and b["access_tier"]=="A1" and b["evidence_level"]=="REPRODUCED"
 ...
 o=b["observations"]; pr=o["protocol"]; assert pr["context_length"]==128 and pr["prediction_length"]==32 and pr["patch_size"]==16 and pr["patch_stride"]==16 and pr["full_forecast_tensors_persisted"] is False
 ...
 assert o["reference_repeat"]["hash_equal"] is True and o["reference_repeat"]["max_abs_delta"]<=1e-6
 d=b["derived_metrics"]; assert d["reference_repeat_exact"] is True and all(v is True for v in d["portable_method_checks"].values())
 for k in ("positive_amplitude_rescaled_max_delta","positive_amplitude_rescaled_mean_delta","negative_sign_symmetry_max_delta","within_final_patch_median_delta_mean","cross_patch_median_delta_mean"): assert finite(d[k]) and d[k]>=-1e-6
 assert int(d["largest_position_effect"]) in LAGS
 assert b["provenance"]["raw_output_hash"]==hjson({"observations":o,"derived_metrics":d})
 print(json.dumps({"valid":True,"probe_id":b["probe_id"],"identity_digest":ap["identity_digest"],"amplitude_rescaled_max":d["positive_amplitude_rescaled_max_delta"],"negative_symmetry_max":d["negative_sign_symmetry_max_delta"],"within_patch_mean":d["within_final_patch_median_delta_mean"],"cross_patch_mean":d["cross_patch_median_delta_mean"],"largest_position_effect":d["largest_position_effect"],"scientific_outcome_not_acceptance_gate":True},sort_keys=True)); return 0
