...
def compare_repeat(reference:torch.Tensor,candidate:torch.Tensor)->dict[str,Any]:
    return {"hash_equal":th(reference)==th(candidate),"max_abs_delta":float((candidate-reference).abs().max()),"mean_abs_delta":float((candidate-reference).abs().mean()),"cosine_distance":cosine(candidate,reference)}
...
def main()->int:
    ...
    with torch.inference_mode():
        amp=pipe.predict(amp_contexts,prediction_length=HORIZON,limit_prediction_length=True).float().cpu()
        amp_repeat=pipe.predict(amp_contexts,prediction_length=HORIZON,limit_prediction_length=True).float().cpu()
    ...
    with torch.inference_mode():
        zero_f=pipe.predict([zero],prediction_length=HORIZON,limit_prediction_length=True).float().cpu()[0]
        single_ref=pipe.predict([impulse(5.0,0)],prediction_length=HORIZON,limit_prediction_length=True).float().cpu()[0]
    exp=time.perf_counter()-t
    repeatability={
        "same_batch_amplitude_repeat": compare_repeat(amp,amp_repeat),
        "amp5_reference_vs_position_batch_lag0": compare_repeat(ref,lag0),
        "amp5_reference_vs_singleton": compare_repeat(ref,single_ref),
        "position_batch_lag0_vs_singleton": compare_repeat(lag0,single_ref),
    }
    observations={"protocol":{"context_length":CONTEXT_LENGTH,"prediction_length":HORIZON,"patch_size":patch_size,"patch_stride":patch_stride,"quantiles":QUANTILES,"full_forecast_tensors_persisted":False,"repeatability_controls_batch_composition":True},"zero_context":summary(zero_f),"positive_amplitude_sweep":amp_rows,"sign_reversal":sign,"position_sweep":position_rows,"repeatability":repeatability}
    within_nonterminal=[r for r in position_rows if 1<=r["lag_from_end"]<=15]; cross=[r for r in position_rows if r["lag_from_end"]>=16]
    same_batch_exact=repeatability["same_batch_amplitude_repeat"]["hash_equal"] and repeatability["same_batch_amplitude_repeat"]["max_abs_delta"]<=1e-6
    derived={"positive_amplitude_rescaled_max_delta":max(r["rescaled_to_amp5_max_abs_delta"] for r in amp_rows),"positive_amplitude_rescaled_mean_delta":float(np.mean([r["rescaled_to_amp5_mean_abs_delta"] for r in amp_rows])),"negative_sign_symmetry_max_delta":sign["negative_vs_negated_reversed_positive_max_abs_delta"],"nonterminal_within_final_patch_median_delta_mean":float(np.mean([r["median_mean_abs_delta_from_lag0"] for r in within_nonterminal])),"cross_patch_median_delta_mean":float(np.mean([r["median_mean_abs_delta_from_lag0"] for r in cross])),"largest_position_effect":max(position_rows,key=lambda r:r["median_mean_abs_delta_from_lag0"])["lag_from_end"],"same_batch_repeat_exact":same_batch_exact,"cross_batch_position_max_abs_delta":repeatability["amp5_reference_vs_position_batch_lag0"]["max_abs_delta"],"cross_batch_singleton_max_abs_delta":repeatability["amp5_reference_vs_singleton"]["max_abs_delta"],"reference_repeat_exact":same_batch_exact,"portable_method_checks":{"positive_amplitude_sweep_executed":True,"negative_sign_test_executed":True,"within_patch_positions_executed":True,"cross_patch_positions_executed":True,"zero_context_control_executed":True,"same_batch_repeat_observed":same_batch_exact,"cross_batch_repeat_measured":True}}
    raw=hjson({"observations":observations,"derived_metrics":derived}); git=os.getenv("GITHUB_SHA")
    bundle={"probe_id":"chronos-bolt-impulse-response-map-v1","instrument":"timeseries-impulse-amplitude-sign-position-suite","instrument_version":"mvp-2","model_identity":{"repository":UPSTREAM_REPO,"revision":REV,"logical_id":LOGICAL_ID,"model_class":"time-series-probabilistic-forecaster"},"artifact_provenance":{"tracked":True,"foundry_repository":"SemperSupra/model-artifact-foundry","logical_artifact_id":LOGICAL_ID,"upstream_provider":"huggingface","upstream_repository":UPSTREAM_REPO,"upstream_revision":REV,"identity_kind":"oci","identity_digest":digest,"foundry_record_ref":a.candidate_ref,"consumer_selection_ref":f"model-spelunker@{git}" if git else None,"verified":True,"verification_ref":"pullback-verified candidate Foundry hydration with forced-offline local reuse","tokenizer_artifact":None},"access_tier":"A1","evidence_level":"REPRODUCED","claim_tags":["TIME_SERIES","IMPULSE_RESPONSE","PATCH_POSITION","SCALE_INVARIANCE","SIGN_SYMMETRY","BOUNDARY_SEARCH","PROTOCOL_DEPENDENCE"],"observations":observations,"derived_metrics":derived,"uncertainty":{"scope":"single synthetic impulse family; not a general anomaly-response benchmark","negative_scaling":"negative-scale quantile comparison reverses quantile order before comparison","patch_interpretation":"position sweep localizes functional sensitivity relative to declared patch geometry but does not isolate an internal component","repeatability":"bitwise repeatability is tested with identical batch composition; cross-batch numerical differences are preserved as protocol evidence rather than called nondeterminism"},"known_assumptions":["positive impulse amplitude scaling is equivalent to positive scaling of the entire zero-baseline context","negative sign symmetry should compare q with 1-q under exact distributional sign reversal","lag positions around multiples of 16 are informative because input patch size and stride are both 16","same-batch repeat is the appropriate bitwise determinism control"],"known_failure_modes":["a single impulse is out-of-distribution relative to many natural time-series processes","patch-boundary effects can interact with absolute recency and context normalization","functional sensitivity does not establish causal attribution to patching alone","changing batch composition can change floating-point execution and must not be conflated with stochastic model behavior"],"cost":{"model_load_seconds":load,"experiment_seconds":exp,"total_script_seconds":time.perf_counter()-started,"peak_rss_mib":resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/1024.0},"provenance":{"run_id":os.getenv("GITHUB_RUN_ID","local"),"code_revision":git,"model_revision":REV,"tokenizer_revision":None,"environment":{"python":sys.version.split()[0],"platform":platform.platform(),"numpy":np.__version__,"torch":torch.__version__,"chronos_forecasting":importlib.metadata.version("chronos-forecasting")},"randomness":{"deterministic_forecast":True},"raw_input_hash":hjson({"amplitudes":AMPLITUDES,"lags":LAGS,"context_length":CONTEXT_LENGTH,"horizon":HORIZON,"repeat_batches":{"amplitude":len(amp_contexts),"position":len(pos_contexts),"singleton":1}}),"raw_output_hash":raw}}
    a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(bundle,indent=2,sort_keys=True)+"\n")
    print(json.dumps({"output":str(a.output),"amplitude_rescaled_max":derived["positive_amplitude_rescaled_max_delta"],"negative_symmetry_max":derived["negative_sign_symmetry_max_delta"],"nonterminal_within_patch_mean":derived["nonterminal_within_final_patch_median_delta_mean"],"cross_patch_mean":derived["cross_patch_median_delta_mean"],"largest_position_effect":derived["largest_position_effect"],"same_batch_repeat_exact":derived["same_batch_repeat_exact"],"cross_batch_position_max":derived["cross_batch_position_max_abs_delta"],"cross_batch_singleton_max":derived["cross_batch_singleton_max_abs_delta"]},sort_keys=True)); return 0
