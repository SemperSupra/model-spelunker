#!/usr/bin/env python3
"""Test the diagnostically justified MPS nn.Linear -> matmul+bias workaround."""
...
def patch_linears(model:torch.nn.Module)->list[str]:
    patched=[]
...
def run(model,inputs,device:str):
    ...
    return logits,{"model_device":str(next(model.parameters()).device),"logits_device":str(outputs.logits_per_image.device)}
...
def main()->int:
    ...
    # CPU control uses stock nn.Linear.
    ...
    # Native MPS control uses the unmodified model.
    ...
    # Workaround differs only in the nn.Linear implementation on MPS.
    patched_model=AutoModelForZeroShotImageClassification.from_pretrained(
        cfg["model_id"],revision=cfg.get("model_revision")
    )
    patched_names=patch_linears(patched_model)
    ...
    receipt={
        "schema":"visual-concept-mps-linear-matmul-workaround/v1",
        ...
        "placements":{"cpu":cpu_place,"native_mps":native_place,"patched_mps":patched_place},
        "treatment":{
            "name":"mps-nn-linear-matmul-bias-v0",
            "patched_linear_count":len(patched_names),
            "patched_modules":patched_names,
            "provider_fallback":False,
        },
        "rankings":{"cpu":cpu_rank,"native_mps":native_rank,"patched_mps":patched_rank},
        "logit_comparisons":{
    ...
    print(json.dumps({"rankings":receipt["rankings"],"oracle":receipt["oracle"],"patched_linear_count":len(patched_names)},sort_keys=True))
    return 0
