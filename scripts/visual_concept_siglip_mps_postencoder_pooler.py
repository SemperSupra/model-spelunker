...
def select_probes(model: torch.nn.Module) -> dict[str,list[str]]:
    ...
    # Post-encoder pooling/head path is critical for SigLIP. Text uses a
    # normal Linear head; vision uses a MultiheadAttention pooling head whose
    # packed projection path is not covered by nn.Linear.forward patching.
    post = {
        "text": (
            "text_model.head",
        ),
        "vision": (
            ...
            "vision_model.head.mlp.fc1",
            "vision_model.head.mlp.fc2",
            "vision_model.head.mlp",
            "vision_model.head",
        ),
    }
    for tower,suffixes in post.items():
...
def main()->int:
    ...
    receipt={
        "schema":"visual-concept-siglip-mps-postencoder-pooler-localize/v1",
        ...
        "embeddings":{
            "image":metrics(cpu["image_embeds"],patched["image_embeds"]),
            "text":metrics(cpu["text_embeds"],patched["text_embeds"]),
        },
