...
def rtl_oracle(root: Path) -> tuple[bool, str]:
    ...
    rc, out = run_cmd(
        ["iverilog", "-g2012", "-s", "tb", "-o", "sim", "solution.v", "tb.sv"],
        root,
    )
    if rc != 0:
...
"""
    ...
    rc, out = run_cmd(
        ["gcc", "-O0", "-fno-pie", "-no-pie", "-fno-stack-protector", "-o", "challenge", "challenge.c"],
        root,
    )
    if rc != 0:
...
def make_tools(kind: str, task: str, root: Path, trace: list[dict[str, Any]]):
    ...
    @tool
    def read_spec() -> str:
        """Return the exact bounded task specification for this qualification run."""
    ...
    if kind == "rtl":
        ...
        def write_solution(verilog: str) -> str:
            ...
            Args:
                verilog: Complete synthesizable SystemVerilog source for the requested module.
            """
        ...
        @tool
        def run_tests() -> str:
            """Compile solution.v with the fixed testbench and return the bounded test output."""
            passed, detail = rtl_oracle(root)
            result = ("PASS\n" if passed else "FAIL\n") + detail
            return record("run_tests", {}, result)

        return [read_spec, write_solution, run_tests]

    if kind == "binary_re":
        @tool
        def inspect_strings() -> str:
            """Return printable strings from the benign challenge binary."""
            _, out = run_cmd(["strings", "./challenge"], root)
            return record("inspect_strings", {}, out)

        @tool
        def disassemble_main() -> str:
            """Return Intel-syntax disassembly of main from the benign challenge binary."""
        ...
        def try_unlock(code: int) -> str:
            ...
            Args:
                code: Candidate decimal unlock code recovered from the binary.
            """
            rc, out = run_cmd(["./challenge", str(code)], root)
            result = f"exit={rc}\n{out}"
            return record("try_unlock", {"code": int(code)}, result)

        return [read_spec, inspect_strings, disassemble_main, try_unlock]
...
def run_pairing(persona: dict[str, Any], workload: dict[str, Any], repetition: int) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="persona-qualification-") as td:
        root = Path(td)
        ...
        model = LiteLLMModel(
            model_id=f"ollama_chat/{persona['controller']['model']}",
        ...
        tools = make_tools(workload["kind"], workload["task"], root, trace)
        agent = ToolCallingAgent(
            tools=tools,
            model=model,
            instructions=persona["instructions"],
            max_steps=persona["limits"]["max_steps"],
            verbosity_level=0,
        )
        ...
        final_answer = ""
        try:
            final_answer = str(agent.run(workload["task"]))
...
def main() -> int:
    ...
    expected_ids = {
        p["controller"]["expected_manifest_id_prefix"]
        for p in plan["personas"]
    }
    identity_ok = len(expected_ids) == 1 and resolved_model_id in expected_ids
    ...
    qualification_supported = execution_complete and all(
        stats["passed"] >= 1 for stats in by_pairing.values()
