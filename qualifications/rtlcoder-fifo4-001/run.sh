...
input_paths={
    ...
    "run.sh":Path("run.sh"),
    "run_trial.py":Path("run_trial.py"),
    "specialist_adapter.py":Path("specialist_adapter.py"),
...
for name,p in input_paths.items():
    inputs[name]=ident(p)
outputs={}
for name in ("plan.json","result.json","specialist-provenance.json","controller-provenance.json","create-preflight-request.json","create-preflight-response.json","create-preflight-status.txt","rtlcoder-create-request.json","rtlcoder-create-response.json","rtl-fifo4-spec.md","rtl-fifo4-tb.sv"):
    p=out/name
    if p.is_file():
